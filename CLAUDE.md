...
本プロジェクトは、Claude Codeによるスペック駆動開発で書籍を執筆する。
...
命名規則:
- 章の執筆: `20260305-ch04-writing` 形式
- サンプルコード開発: `20260305-sample-code-[機能名]` 形式
...
- **chapter-specs/** - 各章の仕様書（サンプルコード要件含む）
- **figure-list.md** - 図表一覧（全章横断）
- **repository-structure.md** - リポジトリ構造
- **sample-code-spec/** - サンプルコード仕様（該当する場合）
  - `ideas/app-concept.md` - サンプルコード構想
...
2. `/setup-book` で永続的ドキュメント作成（book-concept.mdを元に）
   - 分量（ページ数）の指定: `/setup-book --pages 200`
   - アイデアファイルに分量の記述がある場合はそちらが優先される
   - どちらもない場合は対話的に確認される
...
# サンプルコードの再検証
> /deploy-sample-code 4 --env local

# 自由な編集
> ch04の4.3節を見直して、具体例を追加して
> 用語集に新しい用語の定義を追加して
...
- 概念理解に必要な最小限のコードのみ
- 言語は書籍企画書（`docs/book-plan.md`）で定義された言語を使用
- コードブロックには言語名を指定
- **動作検証の原則**: 概念的な説明（疑似コード）を除き、原稿に掲載するコード例は全て動作検証済みであること
  - `/write-chapter` のサンプルコード構築ステップで検証済みのコードを使用する
  - 検証環境が未設定の場合は静的レビュー（構文チェック、ベストプラクティス確認）で代替
  - 疑似コードにはコメントで「疑似コード」と明示する
- **ソースファイルの明示**: サンプルコードから引用するコード例には「**リストX.Y: `ファイルパス`**」形式でキャプションを付ける（X=章番号、Y=章内連番）
...
レビュー時に自動起動されるサブエージェント:

| エージェント | 役割 | 起動元 |
|---|---|---|
| `chapter-reviewer` | 仕様整合性・用語統一・文体等の品質レビュー | `/write-chapter`, `/review-chapter`, `/fix-feedback` |
| `tech-verifier` | Web検索による技術的正確性の検証 | `/review-chapter`, `/write-all` |
| `citation-verifier` | 情報ソースの有無と正確性の検証 | `/review-chapter`, `/write-all` |
| `quiz-format-checker` | 理解度チェック問題のフォーマット検証 | `/review-chapter`, `/write-all` |
| `mermaid-syntax-checker` | Mermaid記法の構文エラーと配色ルール準拠の検証 | `/review-chapter`, `/write-all` |
| `sample-code-verifier` | サンプルコードのE2E検証 | `/write-chapter`, `/write-all` |

サブエージェントの詳細は `.claude/agents/` を参照。

## 検証環境（任意）

サンプルコードの動作検証環境。設定しない場合、コード例は静的レビュー（構文チェック、ベストプラクティス確認）のみ行う。

- **接続情報**: {環境への接続方法}
- **デプロイ方法**: {デプロイ手順またはスクリプトのパス}
- **クリーンアップ方法**: {リソース削除手順}
