---
name: figure-design
description: Mermaid図やテキスト図を作成・修正する際に使用。配色ルール・図表番号・キャプション形式を定義。章執筆時にchapter-writingスキルから参照される。
---

# 図表設計スキル

書籍に含める全ての図表の設計・配色・番号付けのルールを定義する。

## Mermaid配色ルール（グレー背景対応）

Mermaid図はグレー背景上でレンダリングされるため、デフォルト配色では文字が読めない。全てのMermaid図に以下のテーマ設定を適用すること。
...
| サブ要素 | オレンジ `#E8913A` | サブコンポーネント、ワーカー |
...
graph TB
    ...
    Orch --> CompA[コンポーネントA]:::sub
    Orch --> CompB[コンポーネントB]:::sub
    CompA --> Tool1[ツール1]:::tool
    CompB --> Tool2[ツール2]:::tool
